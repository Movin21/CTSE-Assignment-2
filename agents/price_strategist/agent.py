...
HARD CONSTRAINTS:
  ...
  3. Use only the values returned by the tool â€” do not manually adjust or round the output.
...
def run_pricing_agent(state: GlobalState) -> GlobalState:
    ...
    if not inventory:
        log_event("AGENT_WARN", "PriceStrategist", "No inventory â€” skipping pricing")
        state["pricing_logic"] = {}
    ...
    for item in inventory:
        ...
        if response.tool_calls:
            ...
            log_event("AGENT_WARN", "PriceStrategist",
                      f"LLM did not call tool for '{product_name}' â€” forcing direct call")
